	...
	<div class="<?php echo get_theme_mod('layout_container', 'container'); ?> p-y-1">
		...
		<?php if( $query_about->have_posts() ) : while( $query_about->have_posts() ) : $query_about->the_post(); ?>
			...
			<?php if( $query_contact->have_posts() && $query_about->have_posts() ) : ?>
				<div class="col-xs-12 text-xs-center text-md-right col-md-4 p-y-1 pull-md-right contact-adress secondary-color nav-font">
			...
			?>
				...
				<div class="adress">
					<span><a class="secondary-color" href="http://maps.google.com/?q=<?= str_ireplace(' ', '+', trim($adress)); ?>"> <?= $adress; ?> </a></span>
			...
			<?php else: ?>
			<div class="col-xs-12 text-xs-center col-md-4 p-y-1 contact-socialmedia">
			<?php endif; ?>
				<ul>
				<?php if ($facebook): ?>
					<li>
